//
// Copyright (C) 2018 Dr. Michael Steffens
//
// SPDX-License-Identifier:		BSL-1.0
//


#ifndef OS_Keystone_V3_Model_Endpoint_INCLUDED
#define OS_Keystone_V3_Model_Endpoint_INCLUDED

#include "OS/Keystone/V3/Model/Links.h"
#include <JXXON/Serializable.h>
#include <string>

namespace OS {
namespace Keystone {
namespace V3 {
namespace Model {

struct Endpoint : public JXXON::Serializable
{
	Endpoint();
	Endpoint(const JXXON::Json &json);
	virtual JXXON::Json toJson() const override;

	std::shared_ptr<std::string> id;
	std::shared_ptr<std::string> interface;
	std::shared_ptr<std::string> regionId;
	std::shared_ptr<std::string> serviceId;
	std::shared_ptr<std::string> url;
	std::shared_ptr<Links> links;
};

} // namespace Model
} // namespace V3
} // namespace Keystone
} // namespace OS

#endif // OS_Keystone_V3_Model_Endpoint_INCLUDED
